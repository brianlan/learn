...
def area(a: np.array) -> np.ndarray:
    """
    Args:
        a (np.array): of shape (N, 4), coordinates in box: [xmin, ymin, xmax, ymax]
    ...
    w = (a[:, 2] - a[:, 0]).clip(min=0)
    h = (a[:, 3] - a[:, 1]).clip(min=0)
    return w * h


def iou(a: np.ndarray, b: np.ndarray) -> np.ndarray:
    """Pairwise IOU, meaning a and b should have the same number of boxes
    Args:
        a (np.ndarray): of shape (N, 4), coordinates in box: [xmin, ymin, xmax, ymax]
                        pls. note, (xmax - 1, ymax - 1) is the bottom right pixel
        b (np.ndarray): of shape (N, 4), coordinates in box: [xmin, ymin, xmax, ymax]
                        pls. note, (xmax - 1, ymax - 1) is the bottom right pixel
...
def nms(bboxes: np.ndarray, iou_thresh: float = 0.7) -> Tuple[np.ndarray, np.ndarray]:
    """Computational complexity: O(n^2). No sorting involved.

    Args:
        bboxes (np.ndarray): bounding boxes of shape (N, 5),
                             each bbox consist of [xmin, ymin, xmax, ymax, score]
        iou_thresh (float, optional): Defaults to 0.7.
    Returns:
        Tuple[np.ndarray, np.ndarray]: indexes of kept bboxes and suppressed bboxes
    """
    if bboxes.ndim == 1:
...
def conv2d(
    ...
    Args:
        input (np.ndarray): of shape (N, C_in, H, W)
        weight (np.ndarray): of shape (C_out, C_in, K, K), K is the kernel size.
        bias (np.ndarray): if given, the shape should be (C_out,)
